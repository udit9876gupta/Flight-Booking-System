...
export default function UpdateFlights() {
    ...
    const handleSubmit = event => {
        ...
        axios.put('http://localhost:8084/flights/update/' + inputs.flightId, userObject)
            .then((res) => {
        ...
        <Stack sx={{ m: 1 }} alignItems='center' direction='row' spacing={2}>
            <Card sx={{ minWidth: 345, maxWidth: 745 }}>
                ...
                <CardActions disableSpacing>
                    <Typography color='gray' variant='body2' sx={{ ml: 1 }}>
                        click the arrow to update flights
                    </Typography>
                    <ExpandMore
                        expand={expanded}
            ...
            <Snackbar open={open} autoHideDuration={6000} onClose={handleClose}>
                <Alert onClose={handleClose} severity="success" sx={{ width: '100%' }}>
                    Flight with ID: {inputs.flightId} updated successfully!
                </Alert>
            </Snackbar>
        </Stack>
    );
